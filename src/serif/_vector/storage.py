...
############# I'm looking to delete this. not sure if i need to, but whatever.
...
#######    }
    ...
    @classmethod
    def from_iterable(cls, values: Iterable[Any], typecode, has_nulls = None) -> ArrayStorage:
        """Create from Python iterable using sized type name."""
        if typecode is None:
            raise ValueError(f"Cannot use ArrayStorage for type {typecode}")
        ...
        for val in values:
            if val is None:
                ...
                ################# I dont need this. We can capture the error thrown by array.
