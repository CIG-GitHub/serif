...
# ---------------------------------------------------------------------------
# Table.rename_columns({old: new}) — returns a NEW table (non-mutating)
# ---------------------------------------------------------------------------

def test_Table_rename_columns_single():
	"""rename_columns({old: new}) returns a NEW table with renamed columns."""
	t = Table({'a': [1, 2, 3], 'b': [4, 5, 6], 'c': [7, 8, 9]})

	t2 = t.rename_columns({'a': 'alpha'})
...
def test_Table_rename_columns_not_found():
	...
	with pytest.raises(SerifKeyError, match="Column 'z' not found"):
		t.rename_columns({'z': 'zeta'})


def test_Table_rename_columns_multiple():
	"""A dict renames several columns at once."""
	t = Table({'a': [1, 2, 3], 'b': [4, 5, 6], 'c': [7, 8, 9]})
	t2 = t.rename_columns({'a': 'alpha', 'b': 'beta', 'c': 'gamma'})
...
def test_Table_rename_columns_bad_key_changes_nothing():
	...
	with pytest.raises(SerifKeyError, match="Column 'invalid' not found"):
		t.rename_columns({'a': 'alpha', 'invalid': 'oops', 'b': 'beta'})
...
def test_Table_rename_columns_chaining():
	...
	t = Table({'a': [1, 2, 3], 'b': [4, 5, 6]})
	t2 = t.rename_columns({'a': 'alpha'}).rename_columns({'b': 'beta'})
	assert t2.column_names() == ['alpha', 'beta']


def test_Table_rename_columns_ambiguous_name_raises():
	...
	with pytest.raises(SerifKeyError, match="ambiguous"):
		t.rename_columns({'a': 'alpha'})


def test_Table_rename_columns_duplicates_by_index():
	"""Integer keys are the unambiguous escape for same-named columns."""
	with pytest.warns(UserWarning, match="Duplicate column name 'a'"):
		t = Table([Vector([1, 2, 3], name='a'),
		           Vector([4, 5, 6], name='a'),
		           Vector([7, 8, 9], name='a')])
	t2 = t.rename_columns({0: 'x', 1: 'y', 2: 'z'})
	assert t2.column_names() == ['x', 'y', 'z']
	assert list(t2['x']) == [1, 2, 3]
	assert list(t2['z']) == [7, 8, 9]


def test_Table_rename_columns_index_out_of_range():
	t = Table({'a': [1, 2]})
	with pytest.raises(SerifKeyError, match="out of range"):
		t.rename_columns({5: 'x'})


def test_Table_rename_columns_getattr_after():
...
def test_Table_rename_columns_preserves_data():
	"""Test that renaming columns doesn't affect the data"""
	t = Table({'a': [1, 2, 3], 'b': [4, 5, 6]})
	t2 = t.rename_columns({'a': 'x', 'b': 'y'})
	assert list(t2['x']) == [1, 2, 3]
