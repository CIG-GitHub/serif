...
All writes happen inside a `with t.mutable() as m:` scope — table
assignment is frozen outside one (the mutation guardrail; the freeze
itself is covered in test_mutable_guardrail.py). The assignment
SEMANTICS under test here are unchanged by the guardrail.
"""
...
class TestListOfColumnsAssignment:
	...
	def test_assign_list_of_vectors_to_subset(self):
		...
		with t.mutable() as m:
			m[:, 1:3] = [Vector([99, 88, 77]), Vector([66, 55, 44])]
	...
	def test_assign_list_shape_mismatch(self):
		...
		with t.mutable() as m:
			with pytest.raises(SerifValueError, match="Shape mismatch"):
				m[:, :] = [[1, 2], [3, 4], [5, 6]]  # Too many columns
...
class TestSliceEdgeCases:
	...
	def test_empty_slice_rows(self):
		t = Table({'x': [1, 2, 3]})
		with t.mutable() as m:
			m[5:10, 'x'] = 999  # Out of range, should be no-op
	...
	def test_unsupported_value_type(self):
		...
		with t.mutable() as m:
			with pytest.raises(SerifTypeError, match="Unsupported assignment value type"):
				m[:, 'x'] = {1: 'a', 2: 'b'}  # Dict not supported
